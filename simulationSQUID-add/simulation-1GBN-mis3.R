...
##
log.name = paste0("log-mis2-1GBN",as.numeric(Sys.time()),".txt")
sink(log.name)
...
for(i in c(1:3,7:9)){ 
  DATA = foreach(r=1:rtimes, .combine = rbind, .packages=c("mnormt","dplyr","metafor"), .errorhandling = "remove")  %dorng%  {
    ...
    ## data with n and log odds
      ...
      integ.limit = 5, 
      cub.tol = 1e-3,
      init.vals = c(set.gr$t.theta+runif(1,-0.1,0.1),set.gr$t.tau+runif(1,-0.1,0.1),set.gr$beta+runif(1,-0.1,0.1)) ## initial value for mu tau and beta
    )
      ...
      func_list = list(
        function(x) with(x, 
                         COPAS1999(yi, n, Pnmax = pmax, Pnmin = pmin,
                                   parset=parset.copas)),
        function(x) with(x, 
                         COPAS2000(yi, vi, set.gr$pmax, Psemin = set.gr$pmin,
                                   parset=parset.copas)),
        function(x) with(x,
    ...
    save(DATA,file = paste0("res-1GBN-mis3/data-set-",i,"-S",S,".RData"))
    message(paste0("Finish-1GBN-mis2-",S,"-",i))
  }}
