...
yvi1 = escalc(measure="PLO", xi=y1, ni=n1, to="only0")
yi1 = yvi1[,1]
vi1 = yvi1[,2]

yvi2 = escalc(measure="PLO", xi=y1, ni=n1, to="all")
yi2 = yvi2[,1]
vi2 = yvi2[,2]
...
res3 = rma(yi1, vi1, data=yvi1)
funnel(trimfill(res3,estimator="R0"), xlim = c(-7,0))
abline(v=res3$beta)
reg3 = regtest(res3, model="lm")
rnk3 = ranktest(res3)
mtext("C. R0 and continuity correction for only studies with 0 ", side = 3, adj = 0)
...
res4 = rma(yi2, vi2, data=yvi2)
funnel(trimfill(res4,estimator="R0"), xlim = c(-7,0))
abline(v=res4$beta)
reg4 = regtest(res4, model="lm")
rnk4 = ranktest(res4)
mtext("D. R0 and continuity correction for all studies", side = 3, adj = 0)
