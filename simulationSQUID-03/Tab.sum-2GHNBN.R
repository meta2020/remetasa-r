...
nset=nrow(set)
for(i in 1:nset){
  load(paste0("res-2GHN-mis2/data-set-",i,"-S",S,".RData"))
...
DF.list = list(DF[,c(1:6,9,12,13,7,10,14,16,8,11,15,17)], 
               DF.cv[,c(1:6,9,12,13,7,10,14,16,8,11,15,17)], 
               tDF[,c(1:6,9,12,13,7,10,14,16,8,11,15,17)])
...
DF.all%>%kbl(., 
         ...
         col.names = c("$S$",     "Patients","T:C", "$\\tau^2$", "$N$",
                       "NN$_P$",  "NN$_O$",  "CN (CP)", "CS (CP)", 
                       "HN$_P$",  "HN$_O$",  "HN$^\\text{Prop}$ (CP)",   "HN$^\\text{HT}$ (CP)",
                       "CBN$_P$", "CBN$_O$", "CBN$^\\text{Prop}$ (CP)", "CBN$^\\text{HT}$ (CP)"),
         align = "r",
