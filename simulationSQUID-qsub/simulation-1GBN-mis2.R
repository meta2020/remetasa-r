...
for(S in s){
for(i in c(1:3,7:9)){ 
  DATA = foreach(r=1:rtimes, .combine = rbind,.errorhandling = c("pass"),.packages=c("mnormt","dplyr","metafor"))  %dorng%  {
    ...
    ## create datasets
      plist = gendata.1bn.t(
        ...
        alpha=set.gr$alpha.1bn, ## <0.05, <0.1, others
        beta=set.gr$beta)
    ...
    ## data with n and log odds
      ...
      if(p==1){
        ...
        res.est = rbind(pnn,pbn,snn,sbn,pnn,pnn,pbn,pbn)
      ...
      htjbn   = c(adj.list[[4]]$mu, adj.list[[4]]$tau, rho=NA, rho.se=NA,
    ...
    save(DATA,file = paste0("res-1GBN-mis2/data-set-",i,"-S",S,".RData"))
