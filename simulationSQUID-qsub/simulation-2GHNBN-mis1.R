...
##
log.name = paste0("log-mis1-2G",as.numeric(Sys.time()),".txt")
sink(log.name)
...
for(i in 1:nrow(set)){ 
  DATA = foreach(r=1:rtimes, .combine = rbind,.packages=c("mnormt","dplyr","metafor"), .errorhandling = "remove")  %dorng%  {
    ...
    parset.nn = list(
      ...
      eps = set.eps,
      init.vals = c(set.gr$t.theta+round(runif(1,-0.5,0.5),2),
                    set.gr$t.tau  +round(runif(1,-0.3,0.3),2))
    )
    ...
    init.vals = c(set.gr$t.theta+round(runif(1,-0.5,0.5),2),
                  set.gr$t.tau  +round(runif(1,-0.3,0.3),2), 
                  set.gr$t.rho  +round(runif(1,-0.3,0.1),2)) ## initials for mu tau and rho
    )
...
##-- Simulation 2: 2-group binomial model based -------
    ...
    DATA = foreach(r=1:rtimes, .combine = rbind,.packages=c("mnormt","dplyr","metafor"), .errorhandling = "remove")  %dorng%  {
      ...
      parset.new = list(
        ...
        init.vals = c(set.gr$t.theta+round(runif(1,-0.5,0.5),2),
                      set.gr$t.tau  +round(runif(1,-0.3,0.3),2), 
                      set.gr$t.rho  +round(runif(1,-0.3,0.1),2))
      )
    ...
    save(DATA,file = paste0("res-2GBN-mis1/data-set-",i,"-S",S,".RData"))
    message(paste0("Finish-2GBN-mis1-",S,"-",i,":",Sys.time()))
}}
