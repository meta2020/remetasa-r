...
for(i in 1:nrow(set)){ 
  ...
  save(DATA,file = paste0("res-2GHN-mis2/data-set-",i,"-S",S,".RData"))
  message(paste0("Finish-2GHN-mis2-",S,"-",i,":",Sys.time()))
...
##-- Simulation 2: 2-group binomial model based -------
    ...
    DATA = foreach(r=1:rtimes, .combine = rbind,.packages=c("mnormt","dplyr","metafor"))  %dorng%  {
      ...
      plist = gendata.2bn.t(
        ...
        rho=set.gr$t.rho,
        Pnmax = pmax, Pnmin = pmin,
        alpha = set.gr$alpha.2bn, 
        beta = set.gr$beta) ## weight=1,0.5,0.3)
    ...
    save(DATA,file = paste0("res-2GBN-mis2/data-set-",i,"-S",S,".RData"))
    message(paste0("Finish-2GBN-mis2-",S,"-",i,":",Sys.time()))
}}
