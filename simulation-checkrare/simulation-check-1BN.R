...
file.sources = list.files("../simulationSQUID/Rfn/")
sapply(paste0("../simulationSQUID/Rfn/", file.sources), source)
...
for(i in c(1:3,7:9,13:15)){ 
    ...
    DATA = foreach(r=1:rtimes, .combine = rbind,.packages=c("mnormt","dplyr","metafor"))  %dorng%  {
      ...
      data = lapply(list(pdata, sdata),
                    function(data) escalc(measure="PLO", xi=yi, ni=ni, data=data)
      )
      lpdata = data[[1]]
      lsdata = data[[2]]
      
      p.small = pdata %>%summarise(prop = mean(yi <=3))%>%c()
      s.small = sdata %>%summarise(prop = mean(yi <=3))%>%c()
