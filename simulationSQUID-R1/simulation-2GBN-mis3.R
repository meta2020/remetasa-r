...
##
log.name = paste0("log-mis3-2GBN",as.numeric(Sys.time()),".txt")
sink(log.name)
...
##-- Simulation 2: 2-group binomial model based -------
    ...
    DATA = foreach(r=1:rtimes, .combine = rbind,.packages=c("mnormt","dplyr","metafor"), .errorhandling = "remove")  %dorng%  {
      ...
      nmin = min(lsdata$n)
      nmax = max(lsdata$n)
      pmax = set.gr$pmax #max(lsdata$wi)
      pmin = set.gr$pmin#min(lsdata$wi)
    ...
    save(DATA,file = paste0("res-2GBN-mis3/data-set-",i,"-S",S,".RData"))
    message(paste0("Finish-2GBN-mis3-",S,"-",i,":",Sys.time()))
